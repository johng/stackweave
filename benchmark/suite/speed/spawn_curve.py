...
config: stackweave + go on HUBS cores (GIL off), asyncio/uvloop/greenlet on 1 core
(GIL on) -- same as the cross-runtime report.  Writes results/spawn_curve.json
for the report curve.  STACKWEAVE_SYSMON/PREEMPT off (microbenchmark watchdog noise).

WARM steady-state, boot excluded for ALL runtimes.  The stackweave programs are
invoked with --warm WARM: they run WARM extra in-process passes and report the
best timed pass, so the one-time stackweave.run() scheduler boot is NOT in the timed
...
SAME basis, not a runloom-specific trick.  (Without --warm, stackweave would carry a
...
fixed cost (front-load loop + drain) amortizing; stackweave's residual (~19 ms) is
larger than Go's (~5 ms), so its small-N rates sag more.  The per-spawn SLOPE is
what matters: warm, stackweave's marginal cost per fiber (~440 ns) is within noise of
Go's (~410 ns).

Two stackweave spawn entries are reported:
  runloom_py -- stackweave.fiber_fast: the fair apples-to-apples vs Go's `go f()`
                (a thin Python spawn, no per-spawn work).
  stackweave_c  -- the pure-C c_entry path (no Python frame): the scheduler ceiling.
NOTE: the DEFAULT stackweave.fiber adds the grow-down auto-sizer (small right-sized
...
stackweave.fiber between fiber_fast and grow-down.
"""
import json
import os
import subprocess
...
REPS = 2
WARM = 4   # extra in-process passes for the stackweave programs (scheduler boot excluded)

# label + how to invoke spawn for each runtime
RTS = [
    ("runloom_py", "Stackweave (M:N) — Python fiber"),
    ("stackweave_c",  "Stackweave (M:N) — pure C fiber (c_entry)"),
