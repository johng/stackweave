"""Memory + stack-footprint profiling for stackweave goroutines.

Three lenses, all from stdlib + stackweave_c's own introspection -- no external
tools, so it runs anywhere the ext builds:

  1. stack HWM  -- stackweave *paints* goroutine stacks, so stackweave_c.stats()
...
def recurse_then_yield(depth):
    ...
    stackweave's stack paint-sweep runs at swap-out, so the goroutine must be
...
def stack_hwm():
    ...
    size = stackweave_c.get_stack_size()
    ...
    for depth in (0, 10, 50, 100, 200, 400, 800):
        ...
        stackweave_c.run()
        hwm = stackweave_c.stats()["stack_hwm"]
        points[depth] = hwm
...
def main(argv=None):
    ...
    print("stackweave %s/%s  gil=%s\n"
          % (stackweave_c.backend(), stackweave_c.netpoll_backend(),
