...
run_pool uses under STACKWEAVE_HARNESS_GON=1, where py_index == wid -- and check
whether adjacent (2k, 2k+1) pairs land on the SAME hub thread.

Run twice: STACKWEAVE_PAIR_AFFINITY unset (expect pairs split) and =1 (expect
co-located).
"""
import os
import sys
...
def root():
    stackweave_c.fiber_n(w, N, indexed=True)
...
print("STACKWEAVE_PAIR_AFFINITY={0!r}  H={1}  pairs={2}".format(
    os.environ.get("STACKWEAVE_PAIR_AFFINITY"), H, total_pairs))
