...
| **stackweave (1 hub)** | **21.4K** | 45ms | 67ms | 81ms | 84ms |

- stackweave ≈ **1.6× gevent** (its direct blocking-style analog) and **1.55×
  asyncio**, with the **tightest tail** of the four.
- uvloop's peak is ~12% above stackweave, but its tail blew out (p99.9 410ms, max
  ~8s, hundreds of requests in the overflow bucket) under this connection
  load; stackweave stayed bounded (max 84ms).

## Multi-core, free-threaded 3.13t (stackweave's native runtime)

| runtime | rps | p50 | p99.9 | max | cores |
| --- | ---: | ---: | ---: | ---: | --- |
| asyncio | 11.3K | 87ms | 114ms | 116ms | 1 |
| uvloop | 19.9K | 41ms | 409ms | 8.2s | 1 |
| stackweave (1 hub) | 19.6K | 49ms | 86ms | 88ms | 1 |
| **stackweave (8 hub)** | **121K** | **7.9ms** | 16ms | 42ms | 8 |

stackweave **scales ~6.2× to 8 hubs** — multi-core that gevent/uvloop/asyncio
