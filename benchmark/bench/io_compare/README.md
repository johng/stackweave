# I/O concurrency comparison — stackweave vs gevent / uvloop / asyncio
...
- `server_gevent.py`  — greenlet + libev, blocking-style (single core) —
  stackweave's **direct competitor**.
- `server_runloom.py`    — stackweave stackful goroutines; `H` hubs (multi-core on 3.13t).
...
four-way single-core comparison runs on **GIL'd 3.13**; stackweave's multi-core
...
# stackweave: STACKWEAVE_SRC=<repo>/src PYTHONPATH=$STACKWEAVE_SRC $GP server_runloom.py host port io H
```
