...
Measures stackweave.blocking() round-trips/sec on the M:N scheduler.  Env knobs:
  ...
  STACKWEAVE_BLOCKPOOL_SHARDS   submit shards (1 = legacy single queue; default = #hubs)
  STACKWEAVE_BLOCKPOOL_WORKERS  total pool workers (default = shards * 3)

Run:  PYTHON_GIL=0 PYTHONPATH=src python3 bench/bench_offload.py
A/B the two scaling fixes:
  STACKWEAVE_BLOCKPOOL_SHARDS=1 ...   # legacy submit convoy
  (default)                        # sharded
...
def main():
    ...
    if DIRECT:
        ...
        def loop():
            for _ in range(each): stackweave.blocking(work)
    for _ in range(FIB): stackweave.fiber(loop)
t0 = time.perf_counter(); stackweave.run(HUBS, main); dt = time.perf_counter() - t0
print("N=%d HUBS=%d %s shards=%s workers=%s : %.3fs  %s ops/s"
      % (N, HUBS, "DIRECT" if DIRECT else "OFFLOAD",
         os.environ.get("STACKWEAVE_BLOCKPOOL_SHARDS","auto"),
         os.environ.get("STACKWEAVE_BLOCKPOOL_WORKERS","auto"),
         dt, format(int(N/dt), ",")))
