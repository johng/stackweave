...
so `active[0] += 1` is a real read-modify-write race -- a stackweave.sync.Lock
...
import stackweave
from stackweave.sync import Lock
...
def main():
    sem = stackweave.Chan(MAX_CONCURRENT)
    ...
    done = stackweave.Chan(NUM_TASKS)
    ...
    def task(tid):
        ...
        try:
            ...
            stackweave.sleep(0.02)               # the protected "slow" section
            with lock:
                active[0] -= 1
        finally:
