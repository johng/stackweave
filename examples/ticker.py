...
stackweave.time.NewTicker(d) delivers a value on its `.c` channel every d
seconds until you Stop() it (stackweave.time also has NewTimer for one-shot,
...
def main():
    ticker = stackweave.time.NewTicker(0.05)
    try:
        for n in range(1, 6):
