...
exists so a stackweave speed claim survives scrutiny.
...
- **Layout-bias guard (on by default).** Every child gets a random-length
  `STACKWEAVE_BENCH_PAD` env var, so layout varies run-to-run and the interval
...
hubs — i.e. stackweave scales cleanly across all cores, mildly coherence-bound, with
