#!/usr/bin/env python3
"""usl.py -- Universal Scalability Law fit for stackweave's M:N scheduler.
...
Fitting (alpha, beta) to measured throughput-vs-hubs tells you which wall stackweave
...
def work(n):
    ...
    stackweave's eval-hook preemption keeps hubs fair (no sysmon "WEDGED" flood),
...
def throughput(hubs, tasks, iters):
    ...
    for _ in range(tasks):
        stackweave_c.mn_fiber(lambda: work(iters))
    stackweave_c.mn_run()
...
def report(pts, alpha, beta, x1):
    print("")
    print("backend : {}".format(stackweave_c.backend()))
...
def main():
    ...
    if hasattr(stackweave_c, "warmup"):
        stackweave_c.warmup(args.tasks * 2)
