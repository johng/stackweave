#!/usr/bin/env python3
"""rigor.py -- statistically rigorous microbenchmark harness for stackweave.
...
Design:
  ...
  * Layout-bias guard: every child gets a random-length ``STACKWEAVE_BENCH_PAD``
...
def run_child(name, inner, warmup, scale):
    ...
    print(json.dumps({
        "workload": name,
        "unit": unit,
        "backend": stackweave_c.backend() if hasattr(stackweave_c, "backend") else "?",
...
def child_env(pin):
    ...
    if not pin:
        ...
        env["STACKWEAVE_BENCH_PAD"] = "x" * random.randint(0, 4096)
    return env
