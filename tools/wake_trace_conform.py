...
The single-thread drain emits its wake-handshake transitions (STACKWEAVE_WAKE_TRACE=
...
TEMPLATE = '''\\* GENERATED by tools/wake_trace_conform.py from a real STACKWEAVE_WAKE_TRACE run.
...
def main():
    ...
    if not raw:
        print("empty trace (was STACKWEAVE_WAKE_TRACE set on a run() offload workload?)")
        return 2
