"""Adversarial network fuzzer for the stackweave.aio bridge (S6).
...
backpressure, accept-loop close). This drives a real stackweave.aio echo server
...
-- S5 -- is caught too):
    ...
    STACKWEAVE_EXTRA_CFLAGS="-fsanitize=address -g" STACKWEAVE_EXTRA_LDFLAGS=-fsanitize=address \\
...
# ---- server subprocess: a real stackweave.aio echo server -------------------------
...
def main():
    ...
    print("fuzzing stackweave.aio echo server on port %d, %d iters" % (port, args.iters))
