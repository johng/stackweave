#!/usr/bin/env bash
# run_pydebug.sh -- run stackweave under a --with-pydebug free-threaded CPython so
...
# then point STACKWEAVE_PYDEBUG_PYTHON at its ./python (in-tree is fine).
#
# Usage:  tools/run_pydebug.sh [ITERS]
# Env:    STACKWEAVE_PYDEBUG_PYTHON   the pydebug free-threaded interpreter
...
echo "================ stackweave under pydebug free-threaded CPython ================"
...
SOAK=$(cat <<'PY'
import gc, os, sys
sys.path.insert(0, os.path.join(os.environ["STACKWEAVE_ROOT"], "src"))
import stackweave_c
...
done=stackweave_c.Chan(NWORK+NCOLL); stop=[False]
def worker():
...
def collector():
    n=0
    while not stop[0]:
        gc.collect(); n+=1; stackweave_c.sched_yield_classic()
    done.send(("gc",n))
def stopper():
...
for _ in range(NCOLL): stackweave_c.mn_fiber(collector)
for _ in range(NWORK): stackweave_c.mn_fiber(worker)
stackweave_c.mn_fiber(stopper)
stackweave_c.mn_run(); stackweave_c.mn_fini()
assert stackweave_c._self_check(0)==0, "self_check"
print("PASS")
PY
)
...
for i in $(seq 1 "$ITERS"); do
    out=$(PYTHON_GIL=0 STACKWEAVE_ROOT="$ROOT" timeout 240 "$PYD" -c "$SOAK" 2>&1 | tail -2)
...
STACKWEAVE_PYDEBUG_PYTHON="$PYD" bash tools/stw_trace_conform_demo.sh 2>&1 | sed 's/^/  /' \
    || { echo "  STW conformance: FAIL"; fails=$((fails+1)); }

echo "----------------------------------------------------------------------------"
[ "$fails" -eq 0 ] && echo "  CLEAN -- stackweave satisfies CPython's internal assertions" \
