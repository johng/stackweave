/*
 * hub_fanout.pml -- the SPANNING model of hub_submit's waker-route fanout
 * (mn_sched_mn_api.c.inc:215-235).  The individual routes are modelled
 ...
 * (A third route, the io_uring loop backend's ring_waiting hint + ring eventfd,
 * was removed with that backend.)
 * Before committing to a wait, a hub ANNOUNCES its mode (idle_waiting
 ...
 *                       idle hint) is stranded; the pump is the backstop.
...
/* The submitter: push, then fan out per the announced hint + the unconditional
 * pump kick (mn_sched_mn_api.c.inc:215-235). */
proctype submitter()
{
...
proctype hub()
{
    ...
    :: mode == PUMP ->
        /* epoll_wait: no idle hint; drains sub_head before blocking, else
