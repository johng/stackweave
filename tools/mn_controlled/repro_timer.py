...
record order.  Same STACKWEAVE_MN_SEED must reproduce one signature.  House style:
.format().
...
for i in range(6):
    stackweave_c.mn_fiber(lambda i=i: worker(i, 0.002))
for j in range(3):
    stackweave_c.mn_fiber(lambda j=j: worker(100 + j, 0.001 * (j + 1)))
