...
the run signature is every consumer's stream in cid order.  Same STACKWEAVE_MN_SEED
...
chans = [stackweave_c.Chan() for _ in range(NCH)]
done = stackweave_c.Chan()      # producers signal completion here
...
def consumer(cid):
    ...
    while openidx:
        idx, (val, ok) = stackweave_c.select([("recv", chans[i]) for i in openidx])
        ci = openidx[idx]
        if not ok:
...
# initial consumers
for cid in range(4):
    stackweave_c.mn_fiber(lambda cid=cid: consumer(cid))

# a spawner goroutine that adds two more consumers mid-run
def spawner():
    stackweave_c.mn_fiber(lambda: consumer(100))
    stackweave_c.mn_fiber(lambda: consumer(101))
stackweave_c.mn_fiber(spawner)

# producers: each channel gets two producers with distinct bases
def boot():
    for ci in range(NCH):
        stackweave_c.mn_fiber(lambda ci=ci: producer(ci, ci * 2 + 1))
        stackweave_c.mn_fiber(lambda ci=ci: producer(ci, ci * 2 + 2))
    # join ALL producers (deterministic) before closing -- no send/close race
    for _ in range(NPROD):
        done.recv()
    for c in chans:
        c.close()
stackweave_c.mn_fiber(boot)
