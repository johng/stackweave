...
REAL stackweave run (STACKWEAVE_STW_TRACE=<path>, one ndjson line each, keyed by the
...
real gc-churn run against the actual spec -- conforming stackweave's interaction with
...
def main():
    ...
    if not events:
        print("empty trace -- nothing to check (was STACKWEAVE_STW_TRACE set?)")
        return 2
    ...
    # runs CHURN: transient tstates -- stackweave rescue tstates and short-lived native
...
TEMPLATE = '''\\* GENERATED by tools/stw_trace_conform.py from a real STACKWEAVE_STW_TRACE run.
