...
A controlled scheduler for stackweave's single-hub `run()` path, enabled by
`STACKWEAVE_PCT_SEED`. Instead of FIFO, the ready-pop runs the highest-priority ready
...
Off by default — one predicted-not-taken branch, zero cost when `STACKWEAVE_PCT_SEED`
is unset. Env: `STACKWEAVE_PCT_SEED`, `STACKWEAVE_PCT_DEPTH` (d, 3), `STACKWEAVE_PCT_STEPS` (k,
2000), `STACKWEAVE_PCT_DEBUG`.
...
- **reach the M:N hubs** — those are real parallel OS threads; PCT can't
  serialize them, and that's where stackweave's hardest concurrency bugs live. (For
  the M:N path: `tools/mn_stress.py`, `exttsan`, `tools/verify/`, and the controlled
  M:N scheduler `STACKWEAVE_MN_SEED` / `tools/mn_controlled/`.)
...
So the aio bridge marks every such goroutine `fifo` (`stackweave_c.fiber(fn,
...
an earlier one. Result: PCT is safe to run on `stackweave.aio` (asyncio test surface
...
## Logical clock for timers (`STACKWEAVE_LOGICAL_CLOCK`)

To make a TIMER schedule replay deterministically (and make `loop.time()` exact),
`STACKWEAVE_LOGICAL_CLOCK=1` measures `sched_sleep` deadlines + the aio `loop.time()`
...
time. Combine with `STACKWEAVE_PCT_SEED` for full order+time deterministic replay.
...
replay across hubs), which is where stackweave's concurrency risk actually
