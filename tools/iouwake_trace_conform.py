...
its transitions (STACKWEAVE_IOUWAKE_TRACE=<path>): SUBMIT (an SQE submitted + the
...
TEMPLATE = '''\\* GENERATED by tools/iouwake_trace_conform.py from a real STACKWEAVE_IOUWAKE_TRACE run.
...
def main():
    ...
    if not raw:
        print("empty trace (was STACKWEAVE_IOUWAKE_TRACE set on an io_uring "
    ...
    if not episodes:
        print("no io_uring CQE-wake episodes in the trace (did any TCPConn.recv "
              "run under STACKWEAVE_TCPCONN_IOURING=1 on the single-thread drain?)")
        return 2
