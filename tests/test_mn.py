...
stackweave's channels, select, sleep, and work-stealing all behave differently
...
stackweave_c.mn_fiber(setter)     # ... then the never-run fiber they wait on
stackweave_c.mn_run()
stackweave_c.mn_fini()
assert flag[0]
assert stackweave_c._self_check(0) == 0
...
def setter():
    flag[0] = True
stackweave_c.mn_init(1)
stackweave_c.mn_fiber(setter)     # fresh -> goes to the deque
stackweave_c.mn_fiber(waiter)     # popped first (deque LIFO); must yield to setter
stackweave_c.mn_run()
stackweave_c.mn_fini()
assert flag[0]
print("PASS")
...
totals = stackweave_c.Chan(NPAIRS)
def pair(pid):
...
for pid in range(NPAIRS):
    pi, po = pair(pid)
    stackweave_c.mn_fiber(pi); stackweave_c.mn_fiber(po)
stackweave_c.mn_run()
got = 0
...
for _ in range(500): stackweave_c.mn_fiber(w)
stackweave_c.mn_run()
stackweave_c.mn_fini()
v = stackweave_c._self_check(1)
assert v == 0, v
print("PASS")
""")
...
def experiment(nhubs, nprod, ncons, nchan, per):
    chans = [stackweave_c.Chan([0, 1, 8][i % 3]) for i in range(nchan)]
    done = stackweave_c.Chan(nprod)
    res  = stackweave_c.Chan(ncons)
    def producer(pid):
        def run():
    ...
    def consumer():
        ...
        while not all(closed):
            ...
            if not cases:
                break
            idx, (v, ok) = stackweave_c.select(cases)   # BLOCKING select
    ...
    for _ in range(ncons): stackweave_c.mn_fiber(consumer)
    for p in range(nprod):  stackweave_c.mn_fiber(producer(p))
    stackweave_c.mn_fiber(closer)
    stackweave_c.mn_run()
    rc = rt = 0
    for _ in range(ncons):
...
def run_once(nrelay, n):
    src  = stackweave_c.Chan(0)
    dst  = stackweave_c.Chan(0)
    ...
    def relay(a, b):
        ...
        def run():
            ...
            while True:
                ...
                idx, payload = stackweave_c.select(cases)
                if idx == 0:
                    v, ok = payload
                    if not ok:
    ...
    stackweave_c.mn_fiber(relay(src, dst))
...
def experiment(it):
    ...
    def cons():
        ...
        while not all(closed):
            ...
            if not cs:
                break
            idx, (v, ok) = stackweave_c.select(cs)
            live = [i for i in range(nchan) if not closed[i]]
            if ok:
                c += 1
            else:
