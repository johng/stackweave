...
stackweave IS a Go-style M:N scheduler (single sched + N OS-thread hubs, a Chase-Lev
...
for _ in range(8):                 # spinners monopolize the hubs first
    stackweave_c.mn_fiber(spinner)
for k in range(NWORK):             # then the workers they could starve
    stackweave_c.mn_fiber(worker(k))
