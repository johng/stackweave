...
pushed to their edges at once:
  ...
  * STACKWEAVE_SWEEP_MAX_CHURN caps how many dead-fd registration links the
  ...
  * STACKWEAVE_NETPOLL_MAXFD sizes the per-fd arm-cache / pending-wake tables (set
...
still reaps the backlog (no link/fd leak).  STACKWEAVE_DBG_NETPOLL=1 traces the
...
Stresses: Stresses: netpoll dead-fd link sweep under a high-churn short-lived-connection storm with STACKWEAVE_SWEEP_MAX_CHURN capping the sweep rate, and the per-fd arm-cache table (STACKWEAVE_NETPOLL_MAXFD) exercised with fd numbers near the configured cap; correctness of every round-trip despite sweep backlog.
"""
import os
import socket
import struct
...
# extension at netpoll init / sweep time, so they MUST be set before stackweave_c
...
os.environ.setdefault("STACKWEAVE_NETPOLL_MAXFD", "4096")
os.environ.setdefault("STACKWEAVE_SWEEP_MAX_CHURN", "64")
...
def idle_holder(H, addr, hold):
    ...
    try:
        ...
        while H.running():
            if stackweave_c.wait_fd(s.fileno(), 1, 250) & 1:
                if not s.recv(1):
...
def setup(H):
    ...
    # the small STACKWEAVE_NETPOLL_MAXFD cap.  Bounded by IDLE_POOL and never grows.
    hold = []
    addr = (host, port)
...
def post(H):
    ...
    H.log("round_trips={0} maxfd={1} sweep_max_churn={2} idle_pool={3}".format(
        H.total_ops(), os.environ.get("STACKWEAVE_NETPOLL_MAXFD"),
        os.environ.get("STACKWEAVE_SWEEP_MAX_CHURN"), IDLE_POOL))


if __name__ == "__main__":
    if stackweave_c.netpoll_backend() != "epoll":
        print("SKIP: netpoll churn-sweep + MAXFD arm-table test is Linux/epoll "
              "specific (backend={0})".format(stackweave_c.netpoll_backend()))
        sys.exit(0)
    harness.main("p230_netpoll_churn_sweep_maxfd", body, setup=setup, post=post,
                 ...
                 describe="high-churn one-RT connection storm with a THROTTLED "
                          "dead-fd sweep (STACKWEAVE_SWEEP_MAX_CHURN) and a SMALL "
                          "arm-table (STACKWEAVE_NETPOLL_MAXFD); idle pool pushes "
