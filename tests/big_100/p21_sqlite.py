...
blocking-worker pool via stackweave.blocking, since sqlite3 is a GIL-releasing C
...
MAX_FUNCS cap (sqlite library ceiling, NOT a stackweave limit): each worker opens
...
worker; fault 0x1).  The stackweave blocking-pool handoff is provably ordered
...
sqlite's SHARED-WAL concurrent close, not the stackweave blocking pool.
...
def worker(H, wid, rng, state):
    ...
    try:
        ...
        while H.running():
            ...
            try:
                cnt, total = stackweave.blocking(step)
            ...
            if seq >= CYCLE:
                try:
                    stackweave.blocking(lambda: (con.execute(
