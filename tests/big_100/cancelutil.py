...
`g.cancel()`.  stackweave's idiom is cooperative cancellation: a `context` whose
`done` channel closes on cancel, which goroutines watch with `select`.  These
helpers wrap that pattern (plus timeouts via `stackweave.time.After`).
"""
import stackweave
import stackweave.context as _ctx
import stackweave.time as _time
...
def cancellable_sleep(ctx, seconds):
    ...
    idx, _payload = stackweave.select([("recv", ctx.done), ("recv", timer)])
    return idx == 1
