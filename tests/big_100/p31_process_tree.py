...
# Child: spawn N grandchildren that sleep (the Unix `sleep` binary), then sleep
# itself.  killpg takes the whole group atomically, so the long sleep never
# leaks.
PARENT = (
    ...
    while H.running():
        ...
        try:
            proc = procutil.popen(
                ...
                running=H.running, start_new_session=True)
