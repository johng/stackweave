...
owned instances -> cross-hub refcount per op.  STACKWEAVE_IMMORTALIZE_SHARED=1
...
def setup(H):
    ...
    if os.environ.get("STACKWEAVE_IMMORTALIZE_SHARED", "1") == "1":
        ...
        stackweave_c.immortalize(H.state["lock"])
        stackweave_c.immortalize(H.state["counter"])
