#!/usr/bin/env bash
# Build + run the ATTACHED/CPU preemption repro (STACKWEAVE_PREEMPT).  Same binary,
# env discriminator: no STACKWEAVE_PREEMPT -> RED (CPU-bound Python g monopolises its
# hub); STACKWEAVE_PREEMPT=1 -> GREEN (eval-frame wrapper yields it).  Logs to
# preempt_test.log.
set -u
...
echo "--- RUN no-lost-wake check (STACKWEAVE_PREEMPT=1) -- expect PASS 64/64 ---"
rc=0
for r in 1 2 3 4 5; do
    PYTHON_GIL=0 STACKWEAVE_PREEMPT=1 STACKWEAVE_SYSMON_MS=20 timeout 30 ./test_preempt || rc=1
    echo "  run $r exit rc=$?"
done
