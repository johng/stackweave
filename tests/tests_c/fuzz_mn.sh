...
# STACKWEAVE_DEBUG_DIAG=all so the lifecycle event ring is dumped to stderr
...
for s in $(seq 1 "$MAX"); do
    if ! timeout 15 "$BENCH" "$N" "$H" "$M" "$s" > /tmp/fuzz_mn.$$.out 2>&1; then
        ...
        echo
        echo "[fuzz] reproducing with STACKWEAVE_DEBUG_DIAG=all ..."
        STACKWEAVE_DEBUG_DIAG=all timeout 20 "$BENCH" "$N" "$H" "$M" "$s"
