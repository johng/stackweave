#!/usr/bin/env python3
"""bench_keepalive_py.py -- steady-state keepalive workload (T3.1 stackweave target).
...
# Deep-then-shallow knob: STACKWEAVE_BENCH_DEPTH makes the handler recurse this
...
BENCH_DEPTH = int(os.environ.get("STACKWEAVE_BENCH_DEPTH", "0"))
...
def server_conn(conn, idx):
    ...
    try:
        while True:
            ...
            stackweave_c.sched_sleep(_db_latency_s(idx * 131 + seq))   # "DB"
            seq += 1
        ...
        if f2 >= 0:
            try:
                stackweave_c.netpoll_unregister(f2)
            except (AttributeError, OSError):
                pass
        try:
            conn.close()
...
def accept_loop():
    ...
    while accepted < N and time.monotonic() < MEASURE_END_T:
        try:
            ...
            stackweave_c.wait_fd(lfd, READ, 50)
            continue
        except OSError:
            break
        while True:
            ...
            cidx = accepted
            accepted += 1
            stackweave_c.mn_fiber(lambda c=conn, i=cidx: server_conn(c, i))
            if accepted >= N:
                break
            try:
                conn, _ = listen_sock.accept()
...
def _client_body(idx):
    ...
    if RAMP_S > 0.0 and N > 1:
        stackweave_c.sched_sleep((idx / float(N)) * RAMP_S)
    fd = s.fileno()
    try:
        s.connect((HOST, PORT))
    ...
    while time.monotonic() < MEASURE_END_T:
        ...
        if MEASURE_START_T <= t1 < MEASURE_END_T:
            mine.append(t1 - t0)
        stackweave_c.sched_sleep(THINK_S)        # think-time: parked + idle
    _rst_close(s)
    return True
...
def main(argv):
    ...
    # STACKWEAVE_BENCH_STACK=<bytes>: pin the goroutine stack size (freezes
    # calibration) so we can measure RSS vs stack allocation size.
    _bench_stack = os.environ.get("STACKWEAVE_BENCH_STACK")
    if _bench_stack:
        stackweave_c.set_stack_size(int(_bench_stack))
    ...
    # STACKWEAVE_DATASTACK_DEBUG): total reclaimable tail bytes seen, how many
    # were resident at madvise time (mincore), and chunk count.
    ds_tail = ds_res = ds_chunks = 0
    try:
        ds_tail, ds_res, ds_chunks = stackweave_c._datastack_sweep_stats()
