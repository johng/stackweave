...
def main():
    ...
    if args.immortalize:
        stackweave_c.immortalize(noop)

    stackweave_c.mn_init(args.hubs)
    b_spawn = b_run = b_total = 1e18
    prev_done = 0  # mn_run() returns the CUMULATIVE completed count; check the delta
    for _ in range(args.reps):
        t0 = time.perf_counter()
        if args.stack_size > 0:
            stackweave_c.fiber_n(noop, args.n, stack_size=args.stack_size)
        else:
            stackweave_c.fiber_n(noop, args.n)
