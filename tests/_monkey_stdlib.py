...
``stackweave.monkey.patch()`` -- the gevent-style "run the real stdlib tests green"
...
which runs CPython's ``test_asyncio`` against ``StackweaveEventLoop``.

Mechanics: each CPython test method is run inside its own stackweave fiber (the
...
# STACKWEAVE_STDLIB_STACK.
STACK = int(os.environ.get("STACKWEAVE_STDLIB_STACK", str(8 << 20)))
...
# run only for boundary exploration with STACKWEAVE_STDLIB_REALTHREAD=1.  The
# cooperative primitives' GOROUTINE-driven behaviour is covered cleanly by the
# hand-adapted tests/test_*_compat.py.
REALTHREAD = os.environ.get("STACKWEAVE_STDLIB_REALTHREAD") == "1"
REALTHREAD_REASON = ("verbatim CPython tests here drive blocking primitives from "
                     ...
                     "exploratory -- set STACKWEAVE_STDLIB_REALTHREAD=1 to run")
...
class MonkeyHosted(object):
    ...
    stackweave fiber on the MAIN thread (setUp / the test / tearDown all in the
    one fiber), driven by the C scheduler the way real ``stackweave.monkey``
    ...
    def run(self, result=None):
        ...
        stackweave_c.fiber(body, stack_size=STACK)
        stackweave_c.run()
