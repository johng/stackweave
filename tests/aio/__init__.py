"""Vendored CPython asyncio conformance suite, pinned + run on stackweave's bridge.
...
swapped to stackweave.aio.StackweaveEventLoop centrally in conftest.py, so the bodies
...
SCOPE: only the submodules that genuinely exercise StackweaveEventLoop are vendored
...
they run on a fake/stock loop and would prove nothing about stackweave.

DEFAULT BRIDGE, no src changes: divergences on the stackweave loop (unimplemented
