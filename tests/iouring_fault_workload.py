...
(stackweave_c.file_read / file_write) from inside a fiber so an error injected
...
def mode_fileread():
    ...
    def worker():
        ...
        try:
            ...
            try:
                n = stackweave_c.file_read(fd, buf, len(PAYLOAD), 0)
                out["n"] = n
...
def mode_badfd():
    ...
    def worker():
        ...
        buf = bytearray(16)
        try:
            stackweave_c.file_read(fd, buf, 16, 0)
            out["unexpected"] = True
        except OSError as e:
            out["errno"] = e.errno
...
def main():
    ...
    print("IOURING_AVAIL=%d" % (1 if stackweave_c.iouring_available() else 0),
          file=sys.stderr)
    if mode == "fileread":
