...
The single most-cited OPEN stackweave bug is the offload wedge (a goroutine parked in
stackweave.blocking() never re-queued at high concurrency: p23/p17 on Linux, p92 on
...
DIRECTLY (in the goroutine) and via stackweave.blocking() (offloaded to a worker
...
def _run(n, hubs):
    ...
    def worker(i):
        try:
            ...
            offloaded = stackweave.blocking(op, seed)  # offload -> park -> resume
            if direct != offloaded:
                mismatches[i & 1023] += 1
            completed[i & 1023] += 1
        finally:
