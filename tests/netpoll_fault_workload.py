...
fiber parks in stackweave_c.wait_fd and a feeder OS thread makes the fd
...
READ = 1
TIMEOUT_MS = int(os.environ.get("STACKWEAVE_FAULT_TIMEOUT_MS", "2000"))
...
def mode_happy():
    ...
    def waiter():
        ...
        try:
            got.append(stackweave_c.wait_fd(r.fileno(), READ, TIMEOUT_MS))
        except OSError as e:
            err.append(e.errno)
...
def mode_timeout():
    ...
    print("DONE got=%r backend=%s" % (got, stackweave_c.netpoll_backend()))
    return 0
