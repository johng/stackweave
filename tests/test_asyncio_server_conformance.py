"""CPython asyncio start_server lifecycle conformance against StackweaveEventLoop.

Companion to test_asyncio_conformance.py.  Runs CPython's OWN
test_server.BaseStartServer verbatim against stackweave.aio.StackweaveEventLoop via the
...
This was a documented gap (stackweave's create_server ignored start_serving and began
...
class RunloomStartServerConformance(_tsrv.BaseStartServer, unittest.TestCase):
    """CPython's BaseStartServer, driven by a StackweaveEventLoop.

    Every test_* method is CPython's, unmodified; only the loop is stackweave's."""
