...
def test_prewarm_synchronous_retains():
    ...
    n = stackweave_c.prewarm(500, 512 * 1024, False)
    assert n == 500, n
...
def test_prewarm_background_returns_immediately():
    ...
    t0 = time.monotonic()
    r = stackweave_c.prewarm(500, 512 * 1024, True)
...
def test_prewarm_default_args():
    ...
    assert stackweave_c.prewarm(100) == 0


def test_fibers_run_after_prewarm():
    stackweave_c.prewarm(800, 512 * 1024, False)   # seed the depot
    done = bytearray(400)
...
def test_prewarm_zero_and_negative_are_noops():
    assert stackweave_c.prewarm(0, 512 * 1024, False) == 0
    assert stackweave_c.prewarm(-5, 512 * 1024, False) == 0


def test_prewarm_keep_start_retarget_stop():
    # Continuous daemon: start, re-target a running one (idempotent), clean stop.
    assert stackweave.prewarm_keep(200, 512 * 1024) == 0   # start
    assert stackweave.prewarm_keep(400) == 0               # retarget running daemon
    time.sleep(0.1)                                      # let it fill some
    stackweave.prewarm_stop()                              # halt + join
    stackweave.prewarm_stop()                              # idempotent no-op
    # restart after stop works
    assert stackweave.prewarm_keep(100) == 0
