"""Concurrency tests for stackweave: race conditions, deadlocks, ordering.
...
class TestParkWakeRace(unittest.TestCase):
    ...
    def test_multiple_wakes_consumed(self):
        ...
        def burst_waker():
            stackweave_c.sched_yield_classic()
            stackweave_c.sched_yield_classic()  # let parker capture itself
            for _ in range(3):
                g_holder[0].wake()

        stackweave_c.fiber(parker)
        stackweave_c.fiber(burst_waker)
        stackweave_c.run()
...
class TestSelect(unittest.TestCase):
    def test_select_with_default_no_block(self):
        ...
        is ready."""
        ch_a = stackweave_c.Chan(1)
        ch_b = stackweave_c.Chan(1)
        result = [None]

        def w():
            r = stackweave_c.select([("recv", ch_a),
    ...
    def test_select_picks_ready_case(self):
        ...
        result = [None]
        def w():
            idx, val = stackweave_c.select([("recv", ch_a),
                                         ("recv", ch_b)])
            result[0] = (idx, val)
        stackweave_c.fiber(w)
        stackweave_c.run()
    ...
    def test_select_send_case(self):
        ...
        def w():
            idx, _ = stackweave_c.select([("send", ch, "x")])
            result[0] = idx
    ...
    def test_select_blocks_until_ready(self):
        ...
        def w():
            idx, val = stackweave_c.select([("recv", ch)])
            out.append((idx, val))
