...
(depot static-cap override, the invariant sanitizer, the prewarm
thread-spawn-failure paths).
...
The key structural fact batch 1 missed:
    ...
    measurement-window MADV reclaim and the depot flush) once that
    per-thread pool OVERFLOWS, i.e. once MORE THAN 512 coros have been
    destroyed ON A SINGLE HUB.

So every stack-RELEASE path -- e.g. the depot over-cap munmap in
runloom_stack_flush_to_global (L403-404) -- is ONLY exercised when >512 coros
are released on ONE pool.  Batch 1 spawned 200-300 at a time, so the pool
absorbed them all and none of these lines ran.
...
depot structures stay consistent (rc._self_check == 0) after the churn.
...
# Same skip rationale as batch 1: coro.c's stack pool / madvise live
...
# ~188 real releases per round -- enough to overflow a tiny depot cap.
CONC = 700
...
# structures are still consistent (rc._self_check == 0) -- a corrupted depot
# flush would trip it.  `rounds` repeats so a second wave of releases hits
# structures already at steady state (depot already at cap for the munmap
# path).
_OVERFLOW = r'''
import stackweave
...
# (keep-resident) release path under a real pool overflow in the parent process.
# --------------------------------------------------------------------------
def test_default_mode_pool_overflow_baseline_in_process():
    ...
    (pooled stacks kept resident, default depot cap): proves the >512 pool-overflow
    ...
    Exercises the default runloom_stack_release path (depot pool insert) under
    a genuine pool overflow."""
    import stackweave
    from stackweave.sync import WaitGroup
