...
deadlock.  stackweave.monkey makes ThreadPoolExecutor fiber-backed, so submitted
        ...
        def body():
            ...
            with cf.ThreadPoolExecutor() as ex:
                fut = ex.submit(lambda: (
                    [stackweave.sleep(0.005) for _ in range(6)], 99)[1])
                val = fut.result()
            done["v"] = True
...
class TestWait(unittest.TestCase):
    def test_wait_all_completed(self):
        def body():
            with cf.ThreadPoolExecutor(max_workers=4) as ex:
                futs = [ex.submit(lambda i=i: (stackweave.sleep(0.005), i)[1])
                        for i in range(6)]
    ...
    def test_wait_first_completed(self):
        def body():
            with cf.ThreadPoolExecutor(max_workers=4) as ex:
                ...
                slow = ex.submit(lambda: ([stackweave.sleep(0.01) for _ in range(10)],
                                          2)[1])
    ...
    def test_wait_timeout(self):
        def body():
            with cf.ThreadPoolExecutor() as ex:
                slow = ex.submit(lambda: ([stackweave.sleep(0.01) for _ in range(20)],
                                          1)[1])
...
class TestAsCompleted(unittest.TestCase):
    def test_as_completed_yields_all(self):
        def body():
            with cf.ThreadPoolExecutor(max_workers=4) as ex:
                futs = [ex.submit(lambda i=i: (stackweave.sleep(0.003 * (i % 3)), i)[1])
                        for i in range(8)]
    ...
    def test_cancel_before_run(self):
        ...
        def body():
            ...
            with cf.ThreadPoolExecutor(max_workers=1) as ex:
                blocker = ex.submit(
                    lambda: ([stackweave.sleep(0.01) for _ in range(5)], 0)[1])
