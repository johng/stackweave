...
Goals:
  - One `pip install .` works across Linux, macOS, FreeBSD/OpenBSD/NetBSD/
    DragonFly, Solaris/illumos, and Android (bionic).
  - Auto-detect compiler family + architecture and pick sane defaults.
  - Degrade gracefully:
      * x86_64 / aarch64  -> asm fast-path (.S)
      * other archs       -> ucontext fallback
...
# file across Linux/macOS/BSD via #ifdef __APPLE__ etc.
# --------------------------------------------------------------------
try:
...
USE_UCONTEXT = (
    ...
    or not (IS_X86_64 or IS_AARCH64)
)
...
def detect_sources():
    ...
    # Arch-specific asm fast path.  Other archs (riscv, ppc, ...) fall
    # through to the ucontext backend via #ifdef in coro.c.
    if not USE_UCONTEXT:
        if IS_X86_64:
...
def detect_compile_args():
    # GCC / Clang / ICC.
    args = [
        "-std=gnu11",          # C11 baseline; gnu* (not strict c11) keeps
                               # cpu_set_t / pthread internals visible on glibc
        "-Wall",
        "-Wextra",
    ...
    if RUNLOOM_FORCE_SELECT:
        # Consumed by plat.h's netpoll selector.
