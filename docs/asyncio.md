# Asyncio bridge (`stackweave.aio`)

`stackweave.aio` lets you run existing `async def` code on the stackweave
scheduler.  It implements the asyncio event-loop protocol on top of
fibers: every `asyncio.Task` becomes a stackweave fiber driving the
coroutine, and `await fut` parks the fiber on a per-task wake
primitive.

**Important:** `stackweave.aio` runs on a **single-threaded scheduler** (one OS
...
`stackweave.aio.run(coro)` is the equivalent of `asyncio.run(coro)`:

1. Installs the stackweave event-loop policy (`StackweaveEventLoopPolicy`).
2. Creates a `StackweaveEventLoop`.
...
print(stackweave.aio.run(main()))   # [0, 2, 4, ..., 18]
```
...
`stackweave.aio` provides `open_connection` and `start_server` that return
...
async def main():
    reader, writer = await stackweave.aio.open_connection("127.0.0.1", 9000)
    writer.write(b"hi\n")
    await writer.drain()
...
async def fetch_one(host, port):
    r, w = await stackweave.aio.open_connection(host, port)
...
The asyncio synchronisation primitives work as-is -- `stackweave.aio` doesn't
reimplement them; they're driven via `Future` and `call_soon`, which
stackweave's loop implements.
...
Low-level fd readiness callbacks work, driven by stackweave's netpoll:

```python
import asyncio, os, stackweave
...
driven by stackweave's netpoll.
...
| | vanilla asyncio | `stackweave.aio` |
| --- | --- | --- |
...
`stackweave.aio` is a high-fidelity bridge for real-world async code -- it runs
...
task is a stackful fiber ordered by stackweave's M:N scheduler (not a callback on
...
> **Note (corrected):** an earlier version of this doc said stackweave "fires most
...
> implied.  See `_fire_callbacks` in `src/stackweave/aio/futures.py`.
...
fires.  stackweave now does the same — `_fire_callbacks` **defers every done-callback
through `loop.call_soon`** to preserve asyncio's order — with exactly two
synchronous exceptions: (a) `StackweaveTask._wake_unpark`, stackweave's own await-wake
...
inside `_run_once`.  stackweave schedules `call_later`/`call_at`/`asyncio.sleep` as
...
- **`loop.time()` is not consulted for firing.**  Mocking `loop.time()` to
  fast-forward a timer (a common asyncio *test* trick) does not advance stackweave's
...
in one strict-FIFO ready deque, one batch per iteration.  stackweave runs them as
...
The useful question when a library's test fails under stackweave: **does it assert an
