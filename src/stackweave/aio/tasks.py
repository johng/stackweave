"""StackweaveTask: the asyncio.Task <-> fiber bridge (heart of the bridge)."""
...
class StackweaveTask(_RunloomFutureMixin, asyncio.Task):
    """A real asyncio.Task subclass (isinstance(x, asyncio.Task) holds) driven
    by a stackweave fiber instead of the C task machinery.
    ...
    def __init__(self, coro, *, loop=None, name=None, context=None,
        ...
        # _cancel_outstanding_tasks): the stackweave scheduler is shared per-thread,
    ...
    def _pg_spawn_driver(self, reraise=False):
        ...
        try:
            self._g = stackweave_c.fiber(body, stack_size=_TASK_STACK, fifo=True) \
    ...
    def _pg_strip_driver_tb(self, exc):
        ...
        cleaner tracebacks free of stackweave internals) means stripping the driver
    ...
    # "<Task pending name=... coro=...>"), shared with StackweaveFuture.
    ...
    def cancel(self, msg=None):
        ...
        if self._self_g is not None:
            # If the fiber is parked in stackweave_c.wait_fd (sock_recv /
    ...
    def _driver(self):
        ...
        self._self_g = stackweave_c.current_g()
        ...
        while True:
            ...
            if yielded is None:
                ...
                # delivers pending socket I/O.  stackweave's sched_yield only
                ...
                try:
                    stackweave_c.netpoll_poll()
                except AttributeError:
                    pass    # older stackweave_c without the non-blocking pump
            ...
            # has ready work; stackweave only pumps netpoll when its ready ring drains
...
def print_tasks(file=None, limit=200, depth=12):
    """Dump every live (not-done) StackweaveTask: where its coroutine is suspended,
    which future it is parked on, and whether that future is done.

    This is the layer stackweave_c.dump_fibers() cannot see.  A task that is
    ...
    file.write("\n=== stackweave aio task dump: %d live (of %d registered) ===\n"
               % (len(live), len(tasks)))
    for t in live[:limit]:
        try:
            name = t.get_name()
