"""StackweaveEventLoop: subprocess_exec/shell, connect_read/write_pipe, and the
...
class _LoopSubprocessMixin(object):
    ...
    def run_in_executor(self, executor, func, *args):
        """Run func(*args) on a thread pool.  Returns a StackweaveFuture
        ...
        fut = StackweaveFuture(loop=self)
        cf_fut = executor.submit(func, *args)
        def _on_thread_done(_cf_fut):
            # Marshal the thread's result back into our StackweaveFuture.
    ...
    # this they can't run under stackweave.  signal.signal must be called from the
