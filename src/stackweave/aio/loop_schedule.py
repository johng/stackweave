"""StackweaveEventLoop: create_task/future, call_soon(_threadsafe), call_later/
call_at, the keepalive timer."""
from ._base import *  # noqa: F401,F403  (shared foundation)
from .futures import StackweaveFuture  # noqa: F401
from .handles import _Handle, _TimerHandle  # noqa: F401
from .tasks import StackweaveTask  # noqa: F401
...
class _LoopScheduleMixin(object):
    def create_task(self, coro, *, name=None, context=None, **kwargs):
        ...
        if self._thread_id is None:
            ...
            task = self._pg_make_task(coro, name, context, kwargs,
                                      defer_spawn=True)
            # ONLY a deferrable StackweaveTask has _pg_spawn_driver.  A custom
            ...
            if isinstance(task, StackweaveTask):
                with self._ts_lock:
                    self._ts_queue.append(_DeferredSpawn(task._pg_spawn_driver))
            return task
        # Foreign thread, loop ALREADY RUNNING: StackweaveTask.__init__ spawns a
    ...
    def _pg_make_task(self, coro, name, context, kwargs, defer_spawn=False):
        ...
        # factory => our own StackweaveTask (the default, fiber-driven path).
        # defer_spawn applies ONLY to the default StackweaveTask; a factory task is
        ...
        if factory is not None:
            ...
            task.set_name(name)
            return task
        # Default path: StackweaveTask ignores any stray kwargs (eager_start etc. --
        # the eager-task factory installs its own factory above).
        return StackweaveTask(coro, loop=self, name=name, context=context,
                           defer_spawn=defer_spawn)

    def create_future(self):
        return StackweaveFuture(loop=self)
    ...
    def _pg_run_loop_cb(self, ctx, callback, args):
        ...
        # enter_task/leave_task.  This matters because a StackweaveTask that parks
        # MID-send (a raw stackweave park inside coro.send -- a blocking offload, etc.,
        ...
        # -- stackweave swallows it as a callback error and the wakeup is DROPPED, so
    ...
    def call_soon(self, callback, *args, context=None):
        ...
        # behaviour is undefined; switch back to stackweave_c.fiber.
    ...
    def call_soon_threadsafe(self, callback, *args, context=None):
        ...
        # We do NOT stackweave_c.fiber() here -- from a foreign thread that would
        ...
        def _keepalive(stop=stop):
            ...
            try:
                while not stop[0] and not self._closed and not self._stopping:
                    ...
                    # ride the logical clock (STACKWEAVE_LOGICAL_CLOCK) -- a logical 2 ms
                    # advances instantly and busy-loops, starving real progress.
                    stackweave_c.sched_sleep_real(0.002)
            ...
            if self._stopping:
                ...
                # run_until_complete's) stackweave_c.run().  sched_stop() acts on
    ...
    def call_at(self, when, callback, *args, context=None):
        ...
        def runner():
            ...
            if not handle._cancelled:
                stackweave_c.sched_sleep(max(0.0, when - self.time()))
            if not handle._cancelled:
                try:
                    ...
                    sys.stderr.write("[stackweave.aio] timer cb: %r\n" % (e,))
        if self._can_spawn_here():
            _fiber_io(runner)
        else:
    ...
    # stackweave MUST mirror that with ONE fiber per fd: a separate fiber per
