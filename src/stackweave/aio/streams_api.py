...
class _Server(object):
    ...
    def _spawn_conn(self, sock):
        ...
        writer = StreamWriter(sock, reader=reader)
        # Build the connection coroutine and drive it directly as a StackweaveTask.
        # We're already inside a non-task fiber (the accept loop or a
        # per-conn TLS fiber); creating StackweaveTask directly here -- the
        # earlier "wrap in stackweave_c.fiber then StackweaveTask inside" added a second
        # fiber spawn for no real benefit.
        coro = self._cb(reader, writer)
        if asyncio.iscoroutine(coro):
            StackweaveTask(coro, loop=asyncio.get_event_loop())
