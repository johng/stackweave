"""_RunloomFutureMixin + StackweaveFuture: synchronous-fire Future replacement."""
from ._base import *  # noqa: F401,F403  (shared foundation)
from .handles import _CANCELLED, _FINISHED, _PENDING, _STOCK_TASK_TYPES, _run_stock_task_cb  # noqa: F401
# StackweaveTask is imported lazily inside _fire_callbacks (below) to avoid a
# futures<->tasks import cycle: tasks subclasses _RunloomFutureMixin from here.

class _RunloomFutureMixin(object):
    """Shared Future logic for StackweaveFuture (over asyncio.Future) and StackweaveTask
    (over asyncio.Task).
    ...
    def __repr__(self):
        # asyncio-compatible repr.  StackweaveFuture/StackweaveTask are drop-in asyncio
        ...
        # Future/Task, not the StackweaveFuture/StackweaveTask implementation class name
    ...
    def __del__(self):
        ...
        # (logging) is fine.
        # Defensive against a half-constructed object: StackweaveTask.__init__
    ...
    # StackweaveFuture's public cancel IS the future-cancel; StackweaveTask overrides it
    ...
    def add_done_callback(self, callback, *, context=None):
        if self._pgstate != _PENDING:
            ...
            # for stackweave's wake timing; only THIS already-done path must defer.)
            loop = self._loop
    ...
    def _fire_callbacks(self):
        from .tasks import StackweaveTask   # lazy: breaks the futures<->tasks cycle
        ...
        for cb, ctx in cbs:
            ...
            # stackweave keeps exactly ONE callback synchronous: StackweaveTask._wake_unpark,
            ...
            sync = (isinstance(host, StackweaveTask)
            ...
            if not sync:
                try:
                    ...
                    self._report_exc(e)
                continue
            # Synchronous: StackweaveTask._wake_unpark, a marked internal control
    ...
    def _report_exc(self, e):
        if self._loop is not None:
            self._loop.call_exception_handler({
                "message": "exception in StackweaveFuture callback",
                "exception": e,
                "future": self,
            })
...
class StackweaveFuture(_RunloomFutureMixin, asyncio.Future):
    """A real asyncio.Future subclass with stackweave's synchronous-callback
...
def _fut_cancelled_error(fut):
    ...
    was cancelled, PRESERVING the future's cancel message.  Both StackweaveFuture and
