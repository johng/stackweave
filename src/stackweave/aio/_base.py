"""Shared foundation for the stackweave asyncio bridge: stdlib re-exports,
...
def _signal_wakeup_noop(signum, frame):
    ...
    # pipe (see StackweaveEventLoop.add_signal_handler), so this is intentionally a
...
# Per-task driver stack size (bytes).  StackweaveTask drivers run arbitrary user
...
# the CPython object tax per task.  Set STACKWEAVE_AIO_TASK_STACK=0 to disable and
# use the scheduler default; set a custom byte count to tune.
try:
    _TASK_STACK = int(_os.environ.get("STACKWEAVE_AIO_TASK_STACK", 512 * 1024))
except ValueError:
...
# roomier stack.  Set STACKWEAVE_AIO_IO_STACK=0 to use the scheduler default; set a
# custom byte count to tune.
try:
    _IO_STACK = int(_os.environ.get("STACKWEAVE_AIO_IO_STACK", _TASK_STACK or 512 * 1024))
except ValueError:
...
def _fiber_io(fn):
    ...
    fifo=True marks the fiber so the PCT controlled scheduler (STACKWEAVE_PCT_SEED)
    ...
    if _IO_STACK:
        return stackweave_c.fiber(fn, stack_size=_IO_STACK, fifo=True)
    return stackweave_c.fiber(fn, fifo=True)


# ------------------------------------------------------------------
# Module-root frame for task-driver fibers.
#
# A StackweaveTask drives its coroutine on the fiber's own swapped C stack,
# whose Python frame chain stackweave_c deliberately severs at the fiber
...
# Under stackweave the walk dead-ends at the driver and AppKey raises
...
def _pg_capture_module_name(default="__main__"):
    """Walk the CREATOR's live stack (StackweaveTask.__init__ runs synchronously on
...
# ------------------------------------------------------------------
# CONCURRENT event loops: one scheduler PER OS THREAD (stackweave "Phase C").
#
# stackweave_c.run() drains the CALLING thread's own (thread-local) scheduler, so
...
#
# The only cross-thread rule: stackweave_c.fiber() (create_task/call_soon/call_later/
...
def _blocking(fn, *args):
    """stackweave_c.blocking (offload fn to the blocking-pool), but deliver a
    cancellation requested WHILE we were in the call.

    stackweave_c.blocking parks the fiber in C with no driver await-point, so
    task.cancel() cannot interrupt it -- it only sets the task's one-shot
    _pgmustcancel and wakes us (which stackweave_c.blocking now ignores until the
    ...
    r = stackweave_c.blocking(fn, *args)
...
# Sentinel stackweave_c.wait_fd returns when the parked fiber was cancelled
...
_WAIT_FD_CANCELLED = getattr(stackweave_c, "WAIT_FD_CANCELLED", 0x40000000)
...
def _wait_fd(fd, events, timeout_ms=-1):
    """stackweave_c.wait_fd, but a cancellation (G.cancel_wait_fd) raises
    ...
    Also preserves asyncio's "current task" across the park.  The StackweaveTask
    ...
    try:
        r = stackweave_c.wait_fd(fd, events, timeout_ms)
    finally:
...
# from StackweaveTask._driver around every send/throw.
try:
...
# for the same loop), so it is safe for stackweave resuming a fiber on a
