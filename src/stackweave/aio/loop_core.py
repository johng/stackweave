"""StackweaveEventLoop: lifecycle, debug, time, and exception-handler/task-
factory plumbing."""
...
class _LoopCoreMixin(object):
    def __init__(self):
        ...
        # loop thread) to break run_forever()/run_until_complete's stackweave_c.run().
        ...
        # re-raises it after stackweave_c.run() returns.  None = none pending.
        ...
        # do NOT enforce thread affinity (stackweave is M:N: callbacks may run
        ...
        # loop.set_task_factory() target.  None => default (build a StackweaveTask);
    ...
    def _timer_handle_cancelled(self, handle):
        ...
        # bookkeeping; stackweave schedules timers as fibers, so it's a no-op.
        pass
    def close(self):
    ...
    def _check_thread(self):
        # No-op: stackweave is M:N, callbacks legitimately run on any hub
    ...
    def time(self):
        ...
        # clock when STACKWEAVE_LOGICAL_CLOCK is on -- so call_at/call_later deadlines
        # line up with sched_sleep's logical deadlines and the timer schedule
        # replays deterministically (e.g. under STACKWEAVE_PCT_SEED).  Identical to
        # _time.monotonic() when the logical clock is off.
        return stackweave_c.loop_clock()
