"""StackweaveEventLoop: run_until_complete/run_forever/stop, the _drive pump,
and shutdown_*."""
...
class _LoopRunMixin(object):
    def _can_spawn_here(self):
        """True iff stackweave_c.fiber is safe on the CALLING thread for THIS loop:
    ...
    def _drive(self):
        ...
        client) freezes only its own sched, never the others'.  stackweave_c.run()
        ...
        try:
            ...
            # StackweaveTask that parked mid-coro.send (its driver's restore-finally
        ...
        # of stackweave_c.run()).  Re-raise it so it propagates out of
        ...
        try:
            if asyncio.iscoroutine(future):
                future = self.create_task(future)
                      ...
                      or isinstance(future, StackweaveFuture)
    ...
    def _cancel_outstanding_tasks(self):
        """Cancel every StackweaveTask still alive on this loop and clear
        ...
        queues so the next stackweave_c.run() sees a clean slate."""
        try:
        ...
        # has live tasks on it.  The stackweave scheduler is one-per-OS-thread, shared
        # by every StackweaveEventLoop on the thread; a blind sched_reset here would
    ...
    def stop(self):
        ...
        # stackweave_c.sched_stop() to return from run_forever()'s stackweave_c.run().
    ...
    def _pg_signal_fatal(self, exc):
        ...
        sched_stop() to return from stackweave_c.run(); _drive re-raises it.
