"""StackweaveEventLoop: add/remove reader+writer, the io-runner fiber, and
...
class _LoopIOMixin(object):
    ...
    def _check_sock_nonblocking(self, sock):
        ...
        # debug stackweave stays lenient and coerces the socket non-blocking below.
    ...
    async def sock_sendfile(self, sock, file, offset=0, count=None, *,
                            fallback=True):
        # No OS sendfile path on stackweave; mirror asyncio's "native unavailable"
        ...
        raise asyncio.SendfileNotAvailableError(
            "sock_sendfile syscall path is not available on stackweave")
    ...
    # loop.  Not part of AbstractEventLoop, but stackweave.monkey makes the blocking
