...
# We list BOTH; StackweaveTask (our own) is excluded at the call site since it wants
...
def _run_stock_task_cb(loop, cb, fut):
    ...
    # slot is empty when a call_soon callback runs.  stackweave CANNOT: StackweaveTask._driver
    # keeps _current_tasks[loop] = self across the whole send/throw, and a task
    # that parks mid-step on a RAW scheduler primitive (stackweave's transport I/O
    # does sock_recv/connect via stackweave_c.wait_fd, not by yielding a future)
    ...
    # "current" StackweaveTask and the stock Task.__wakeup would raise instead of
    # delivering the cancellation (the body-writer hangs forever).
    #
    # The parked StackweaveTask is suspended, not actually executing, so clearing its
    # slot for the duration of the (synchronous) stock-Task step is safe; we
    # restore it afterward so the StackweaveTask's own _driver finally still sees the
...
# Every StackweaveTask, across ALL loops on this process.  The stackweave scheduler is one
# per OS thread (shared by every StackweaveEventLoop on that thread), so loop.close()
...
# Every StackweaveEventLoop that has been constructed and not yet close()'d, across
...
class _Handle(asyncio.Handle):
    ...
    stackweave fires the callback from a fiber after consulting `_cancelled`
...
class _TimerHandle(asyncio.TimerHandle):
    ...
    stackweave schedules via a fiber sched_sleep, not the loop's timer heap."""
...
# ====================================================================
# StackweaveFuture -- pure-Python Future replacement with synchronous-fire
