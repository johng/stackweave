"""stackweave.aio -- async/await on the stackweave scheduler.

Approach: each asyncio.Task gets its own stackweave fiber.  The fiber
...
The wins come from amortizing StackweaveTask setup cost across many awaits.
The losses come from StackweaveTask creation + Chan alloc being heavier
...
Compatibility:
  ...
  * loop.add_reader / add_writer: work (level-triggered like asyncio's
    default selector loop, just driven by stackweave's netpoll).
  * asyncio.start_server / open_connection (Transport+Protocol stack):
    NOT in this MVP -- for I/O, prefer `stackweave.monkey.patch()` and write
...
Use:
    ...
    asyncio.run(main())                  # routed through stackweave
...
# Public API surface (kept importable as stackweave.aio.<name>).
from .loop import StackweaveEventLoop
from .tasks import StackweaveTask
from .futures import StackweaveFuture
...
# Section modules, so any internal name read as stackweave.aio.<name> resolves live
...
def __getattr__(name):
    """Resolve a section-internal name (stackweave.aio._wait_fd, _PG_ALL_TASKS, ...)
    live against the submodules -- preserving the old flat module's read
    surface.  PEP 562 function form on purpose; see stackweave.monkey for why a
...
# ====================================================================
class StackweaveEventLoopPolicy(asyncio.AbstractEventLoopPolicy):
    def __init__(self):
        self._loop = None
    ...
    def new_event_loop(self):
        return StackweaveEventLoop()

    # Child-watcher accessors (deprecated asyncio API still asked for on Unix).
    # stackweave drives subprocesses with its own per-process _wait_thread, NOT an
    # asyncio child watcher, so any watcher set here is INERT -- stackweave never
...
def install():
    """Install StackweaveEventLoopPolicy globally.  After this, every
    `asyncio.run(...)` / `asyncio.new_event_loop()` returns a stackweave
    loop instead of the stdlib selector / proactor loop."""
    asyncio.set_event_loop_policy(StackweaveEventLoopPolicy())


def run(coro, *, debug=False):
    """Drop-in for `asyncio.run`.  Creates a fresh StackweaveEventLoop,
    runs `coro` to completion, returns the result.  Caller doesn't
    need to call install() first."""
    loop = StackweaveEventLoop()
