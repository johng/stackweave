"""StackweaveEventLoop: create_connection/server, unix variants, datagram,
...
class _LoopNetMixin(object):
    ...
    async def sendfile(self, transport, file, offset=0, count=None, *,
        ...
        if not fallback:
            # Caller demanded the native path, which stackweave transports lack.
            raise asyncio.SendfileNotAvailableError(
                "sendfile syscall path is not available on stackweave transports")
        if offset:
