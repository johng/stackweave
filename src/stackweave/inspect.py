"""stackweave.inspect -- see what every fiber is doing.
...
def set_max_fibers(n):
    ...
    the cap, stackweave.fiber / spawn raises RuntimeError -- an admission gate so an
    ...
    when unset; also via env STACKWEAVE_MAX_GOROUTINES."""
...
def watch_leaks(min_age=60.0, interval=30.0, states=PARKED_STATES, on_leak=None):
    ...
    Run this inside your scheduler (stackweave.run / stackweave.aio); it uses cooperative
    sleep, so it never blocks an OS thread."""
    import stackweave as _runloom

    enable_timestamps(True)

    def report(hits):
        sys.stderr.write(
            "stackweave: {0} fiber(s) parked > {1:.0f}s (possible leak):\n"
...
def enable_stack_advice(on=True):
    ...
    ``stackweave_c.fiber(fn, stack_size=...)``, with the guard page + crash reporter
...
def enable_stack_autosize(on=True, prescan=False):
    ...
    seen; once stackweave has measured how much C stack it actually uses, later
...
def print_stack_advice(file=None):
    ...
    file.write("=== stackweave stack advice ({0} kinds) ===\n".format(len(rows)))
...
def format(stacks=True):
    ...
    lines.append("=== stackweave fibers: {0} live ===".format(len(gs)))
...
def dump(file=None, stacks=True):
    ...
    primitive ``stackweave_c.dump_fibers(fd)`` (which is what the SIGQUIT
