"""stackweave.context -- Go-style context.Context for cancellation.
...
Patterns we support, lifted straight from Go:
    ...
    ctx, cancel = stackweave.context.WithTimeout(parent, 5.0)
    ctx, cancel = stackweave.context.WithDeadline(parent, monotonic + 5.0)
...
# Error sentinels (strings -- stackweave deliberately avoids custom exception
...
def WithDeadline(parent, deadline_monotonic):
    ...
    def deadline_waker():
        ...
        # cancel) can wake us early via cancel_wait_fd.  stackweave_c.current_g()
        ...
        ctx._deadline_g = stackweave_c.current_g()
        ...
        while ctx._err is None:
            ...
            if remaining <= 0:
                break
            stackweave_c.wait_fd(_wake_fd(), _READ, int(remaining * 1000) + 1)
        if ctx._err is None:
            ctx._cancel(DEADLINE_EXCEEDED)
