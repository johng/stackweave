...
_tcp_recv_alloc = getattr(stackweave_c, "tcp_recv_alloc", None)
_tcp_recv       = getattr(stackweave_c, "tcp_recv", None)
_tcp_send_once  = getattr(stackweave_c, "tcp_send_once", None)
_tcp_send_all   = getattr(stackweave_c, "tcp_send", None)

_raw_wait_fd = stackweave_c.wait_fd
...
def _wait_fd_coop(fd, ev, timeout_ms=-1):
    ...
    teardown backstop (stackweave_c.cancel_all_parked, audit finding B3) -- makes
...
def _wait_io(sock, fd, direction):
    ...
    Routes through _wait_fd_coop (NOT a bare stackweave_c.wait_fd) so a cross-fiber
...
_orig_close   = None
_orig_detach  = None
_netpoll_unregister = getattr(stackweave_c, "netpoll_unregister", None)
_netpoll_cancel_fd = getattr(stackweave_c, "netpoll_cancel_fd", None)
