...
mapped (one chunk of hysteresis) so it is a benign stale read; under stackweave the
...
no reuse for STACKWEAVE_CHUNK_GRACE further completions), reads each live frame and
...
class _FrameView(object):
    ...
    def __repr__(self):
        ...
        return "<stackweave frame snapshot {0!r} in {1!r}, line {2}>".format(
