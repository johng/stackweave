...
class CoLock(object):
    ...
    Backed by stackweave_c.Mutex (a capacity-1 channel: one buffered token ==
...
class CoRLock(object):
    ...
    def acquire(self, blocking=True, timeout=-1):
        ...
        # stackweave.current() is None) must NOT be mistaken for the owner -- that
    ...
    def release(self):
        cur = stackweave.current() if _in_fiber() else _real_get_ident()
        if self._owner != cur:
            # In a forked child stackweave.current() returns a different G each
