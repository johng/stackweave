"""Shared foundation for the stackweave monkey-patch package: stdlib
...
# our cooperative identity (stackweave.current()) is valid in.  After os.fork() the
# child inherits a *copy* of the scheduler that is not actually running, so
# stackweave.current() there returns a fresh, meaningless G on every call.  This is
...
def _runtime_live():
    """True when a stackweave scheduler is alive on this PROCESS -- and, crucially,
    ...
    if _in_fiber():
        return True
    try:
        return stackweave_c.mn_hub_count() > 0
    except Exception:
        return False
...
class _Parker(object):
    ...
    def __init__(self, inmem=False):
        ...
        # _unpark_all -> stackweave_c.unpark_many for a batched netpoll DIRECT wake (no
        ...
        # re-queues the fiber parked via stackweave_c.park(), and it is set HERE,
    ...
    def park(self, timeout=None):
        if self._via_park:
            ...
            if timeout is None:
                stackweave_c.park(foreign_wakeable=True)
            else:
                stackweave_c.park(foreign_wakeable=True, timeout=timeout)
            return
        ...
        if _in_fiber():
            ...
            # batched stackweave_c.unpark_many instead of an os.write per waiter.
            self._g_handle = stackweave_c.current_g()
...
class CoFMutex(object):
    """Foreign-OS-thread-safe cooperative mutex (drop-in for stackweave_c.Mutex).

    Closes the deadlock CLASS where a foreign OS thread holds the channel-backed
    stackweave_c.Mutex while a fiber locks the SAME mutex: the fiber parks as a
    ...
    CoEvent/CoSemaphore use:
      * a WAITING fiber parks on a 0-fd `stackweave_c.park(foreign_wakeable=True)`
    ...
    stackweave_c.Mutex, never a CoFMutex).
    """
    ...
    def __init__(self):
        ...
        self._guard_mu = stackweave_c.Mutex()    # used SPIN-ONLY (try_lock), never lock()

    # --- spin-only guard: never parks (foreign-safe), never blocks the thread ---
    def _glock(self):
        if self._guard_mu.try_lock():
            return
        if stackweave_c.current_g() is not None:
            while not self._guard_mu.try_lock():
                stackweave_c.sched_yield()        # fiber: yield so the O(1) holder runs
        else:
            while not self._guard_mu.try_lock():
    ...
    # --- drop-in shims for stackweave_c.Mutex / CoLock call sites ---
    def unlock(self):
        self.release()
    ...
    def _at_fork_reinit(self):
        ...
        self._guard_mu = stackweave_c.Mutex()
...
def _unpark_all(parkers):
    ...
    Semaphore).  GOROUTINE waiters are woken by ONE batched stackweave_c.unpark_many
    ...
    if gor:
        missed = stackweave_c.unpark_many(gor)   # one C call for all fibers
        for idx in missed:
...
class _ThreadPoolBackend(_BlockingBackend):
    ...
    SimpleQueue.put() is a single atomic C call; stackweave only preempts at
...
def _offload_hub_ready():
    ...
    try:
        return (stackweave_c.offload_hub_count() > 0 and
                stackweave_c.mn_hub_count() > 0)
    except Exception:
        return False
...
def _submit_offload_hub(fn, args, kwargs):
    ...
    stackweave_c.offload_fiber(_runner)
...
def offload(fn, *args, **kwargs):
    ...
    The sanctioned escape hatch for blocking calls stackweave cannot transparently
    ...
    database drivers, CPU-bound hashing/compression, etc.
        ...
        data = stackweave.monkey.offload(f.read, 65536)
    ...
    serialized -- one in-flight DB call at a time per connection::
        ...
        rows = stackweave.monkey.offload(conn.execute, sql, params).fetchall()
