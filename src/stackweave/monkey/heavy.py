...
# auto-offload only kicks in above a size threshold (STACKWEAVE_OFFLOAD_BYTES, default
...
# have to -- it just happens.  stackweave.monkey.offload() remains for everything else
...
def _heavy_threshold():
    try:
        return int(os.environ.get("STACKWEAVE_OFFLOAD_BYTES", 256 * 1024))
...
# execs deeply-nested untrusted source should use stackweave.monkey.offload() (or a
# roomier g-stack via stackweave_c.fiber(fn, stack_size=...)).
# ============================================================
_orig_compile = None
