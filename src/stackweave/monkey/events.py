...
class CoEvent(object):
    # Waiters park on a _Parker, NOT a stackweave_c.Chan: a channel recv-park is not
    ...
    # untimed -- parks IN MEMORY (stackweave_c.park[(timeout=...)], ZERO per-waiter
    ...
    def __init__(self):
        ...
        self._guard   = stackweave_c.Mutex()      # used SPIN-ONLY (try_lock)

    # --- spin-only guard: never blocks the OS thread ---------------------
    # _guard used to be a real OS lock taken with a BLOCKING acquire.  A real
    # lock blocks the THREAD, and stackweave multiplexes fibers onto hub threads,
    ...
    def _at_fork_reinit(self):
        ...
        self._guard = stackweave_c.Mutex()        # spin-only; see _glock
        self._waiters.clear()
