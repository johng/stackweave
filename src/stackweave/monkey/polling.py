...
# Positive sentinel returned by stackweave_c.wait_fd when the park was cancelled
...
#
# select.select waits on a SET of fds; stackweave's wait_fd parks on ONE fd.  An
...
def _co_select_via_epoll(rlist, wlist, xlist, timeout):
    ...
    try:
        ...
        while True:
            ...
            try:
                rc = stackweave_c.wait_fd(ep.fileno(), READ, wait_ms)  # park on epoll fd
            except OSError:
                raise _SelectFallback()
...
def _co_select_via_kqueue(rlist, wlist, xlist, timeout):
    ...
    try:
        ...
        while True:
            ...
            try:
                rc = stackweave_c.wait_fd(kq.fileno(), READ, wait_ms)  # park on kqueue fd
            except OSError:
                raise _SelectFallback()
            if rc == _WAIT_FD_CANCELLED:
...
def _backing_fd_wait(real_obj, deadline):
    ...
    try:
        rc = stackweave_c.wait_fd(real_obj.fileno(), READ, timeout_ms)
    except OSError:
