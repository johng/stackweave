...
``stackweave.optimize("memory")`` turns it off everywhere to spend that memory back.
...
def _active():
    ...
    # optimize("memory") sets STACKWEAVE_HOT_HANDLERS=0 to spend the RAM back; read
    # live so optimize() can be called before run().
    return os.environ.get("STACKWEAVE_HOT_HANDLERS", "1").strip().lower() not in _OFF
...
#
# Turned on by ``stackweave.optimize("throughput")`` (or ``STACKWEAVE_HOT_AUTO=1``).
# The runtime watches which CLOSURE handlers get spawned a LOT and quietly gives
# the busiest few the ``@stackweave.hot`` treatment -- under a hard budget, so it
...
# ``STACKWEAVE_HOT_AUTO_AFTER`` times (default 64), or past the
# ``STACKWEAVE_HOT_AUTO_BUDGET`` distinct-handler cap (default 32), is left shared;
...
class _AutoHot:
    def __init__(self):
        self.after = _intenv("STACKWEAVE_HOT_AUTO_AFTER", 64)
        self.budget = _intenv("STACKWEAVE_HOT_AUTO_BUDGET", 32)
    ...
    def _warn_budget(self):
        if not self._warned:
            self._warned = True
            warnings.warn(
                "stackweave: per-core hot-handler budget of {0} reached; busier "
                "handlers past it stay shared. Raise STACKWEAVE_HOT_AUTO_BUDGET, or "
                "mark the ones you care about with @stackweave.hot.".format(self.budget),
                stacklevel=2)
...
def _install_auto():
    """Called once from stackweave.run(): enable auto-promotion iff its env is set
    (optimize('throughput') sets STACKWEAVE_HOT_AUTO).  No-op when off."""
    global _AUTO, _AUTO_READY
    if _AUTO_READY:
        return
    _AUTO_READY = True
    if os.environ.get("STACKWEAVE_HOT_AUTO", "0").strip().lower() not in _OFF:
        _AUTO = _AutoHot()
