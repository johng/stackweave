"""stackweave.time -- Go-style time primitives.

Mirrors the subset of Go's `time` package most often used in production:

  stackweave.time.After(d)       -> chan that fires once after d seconds
  stackweave.time.Tick(d)        -> chan that fires every d seconds
  stackweave.time.NewTimer(d)    -> Timer with Stop() / Reset()
  stackweave.time.NewTicker(d)   -> Ticker with Stop() / Reset()
  stackweave.time.Sleep(d)       -> alias for stackweave.sleep

All channels here are unbuffered + Go-style: stackweave_c.Chan.  Consumers
should select on them via stackweave_c.select().
...
def _spawn(fn):
    ...
    stackweave.time primitives are driven by a backing fiber, which must run
...
def Sleep(seconds):
    ...
    Equivalent to Go's `time.Sleep`.  Behaves like stackweave.sleep so
    `from stackweave.time import Sleep` reads naturally: inside a fiber it parks
    ...
    if stackweave_c.current_g() is None:
        # No fiber to park: mirror stackweave.sleep's fallback to the OS sleep.
...
def After(seconds):
    ...
    Idiomatic usage:
        ...
        timeout = stackweave.time.After(5.0)
...
class Timer(object):
    ...
    def _spawn(self, gen):
        d = self._d
        # wait_fd's timeout is in ms; +1 mirrors stackweave.context's waker.  A
        ...
        def fire():
            with self._lock:
                ...
                self._fire_g = stackweave_c.current_g()
            if timeout_ms:
                ...
                stackweave_c.wait_fd(_wake_rfd, _READ, timeout_ms)
            with self._lock:
        ...
        def loop():
            while True:
                with self._lock:
                    ...
                    self._tick_g = stackweave_c.current_g()
