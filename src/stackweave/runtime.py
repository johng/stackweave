"""stackweave runtime: scheduler + fiber helpers.

This module delegates to the C scheduler (stackweave_c.fiber / .run / .sched_*)
...
The public API (stackweave.fiber, .yield_now, .sleep, .run, .current) is preserved
...
#
# On the first stackweave.fiber(fn, ...) spawn we let the C side use the default
...
def set_grow_down(enabled=True):
    ...
    stackweave_c._fiber_set_grow_down(grow_down_active)   # mirror into fiber_grow's C fast path
    ...
    def measured():
        try:
            ...
            if hwm:
                ...
                ceiling = stackweave_c.get_stack_size()   # known-safe cold start
                if want > ceiling:
                    want = ceiling
                if want > store[0]:
                    store[0] = want
...
def _tlbc_reexec_if_needed():
    ...
    PARKED stackweave fiber's suspended frames -- held only in g->snap -- are invisible
    ...
    try:
        import stackweave_c
        anchor_active = bool(getattr(stackweave_c, "gc_frames_active", 0))
    except Exception:
        anchor_active = False
    if anchor_active:
    ...
    os.environ["STACKWEAVE_TLBC_REEXEC"] = "1"
    ...
    try:
        ...
        os.environ.pop("STACKWEAVE_TLBC_REEXEC", None)
        sys.stderr.write(
            "[stackweave] TLBC re-exec failed ({0}); continuing with TLBC ON -- set "
...
def prewarm_stdlib():
    ...
    from stackweave.run() and the aio loop before the scheduler drives any
...
class Goroutine(object):
    ...
    Backed by a stackweave_c.G; the .coro property exposes the underlying
...
def _fiber_full(callable_, *args, **kwargs):
    ...
    ``stackweave.fiber`` is bound to the C ``fiber_grow``, which handles the FROZEN
...
stackweave_c._fiber_register_growdown(GROW_DOWN_KEY, GROW_DOWN_SAMPLES)
stackweave_c._fiber_set_grow_down(grow_down_active)
fiber = stackweave_c.fiber_grow
fiber_fast = stackweave_c.fiber_fast
...
def blocking(fn, *args, **kwargs):
    ...
    not call stackweave scheduler ops (yield/sleep/channels/wait_fd).

    Delegates to stackweave_c.blocking, which runs fn inline when the caller
    isn't on a fiber -- so the same call is safe in either context."""
    return stackweave_c.blocking(fn, *args, **kwargs)


def current():
    """Return the currently-running Goroutine handle, or None.

    Note: returns the bare stackweave_c.G handle, not the Python wrapper.
...
# stackweave_c.mn_hub_count() > 0 test is a check-then-act: two OS threads both
...
def run(n, main_fn=None, offload_hubs=-1):
    ...
    if n == 1:
        ...
        if main_fn is not None:
            ...
            # the stackweave_c.run() below draining an empty local scheduler and
            # returning 0 before main_fn ever ran.  stackweave_c.fiber and
            # stackweave_c.run share the same thread-local scheduler, so main_fn is
    ...
    global _mn_active
    with _mn_lock:
        if _mn_active or stackweave_c.mn_hub_count() > 0:
            raise RuntimeError(
    ...
    try:
        if gil_enabled():
            raise RuntimeError(
                ...
                .format(n))
        prewarm_stdlib()
        stackweave_c.mn_init(n, offload_hubs=offload_hubs)
        try:
            if main_fn is not None:
                stackweave_c.mn_fiber(main_fn)
            return stackweave_c.mn_run()
        finally:
