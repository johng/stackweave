"""stackweave.stats() -- the R0 process-wide gauge surface.

`stackweave_c.stats()` (the C half) reports every internal live population as a
...
backend backlog -- so one `stackweave.stats()` call surfaces every place a leak
can accumulate.
...
def _py_gauges():
    ...
    def _aio_handles():
        from stackweave.aio import handles as h
        return h
    ...
    def _aio_current_tasks():
        from stackweave.aio import _base as b
    ...
    def _dns_entries():
        from stackweave.monkey import dns as d
    ...
    def _mk_base():
        from stackweave.monkey import _base as b
        return b
...
def stats():
    ...
    (stackweave_c.stats()) plus the Python-layer populations (``py_*`` keys).
