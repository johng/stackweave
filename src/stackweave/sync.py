"""stackweave.sync -- plain-Python (no async/await) facade over stackweave.aio.

Same surface, no coroutines.  Lets you port async code to straight-line
synchronous code without giving up the stackweave scheduler.  Two styles:

  Style A: blocking-style I/O inside a fiber.
    stackweave.sync.start()                # opens an implicit scheduler
    sock = stackweave.sync.tcp_connect(...)
    sock.sendall(b'...')              # cooperative
    data = sock.recv(...)
    stackweave.sync.fiber(other_worker)        # other fibers run concurrently
    stackweave.sync.stop()

  Style B: drive a "main" function as the entry point.
    def main():
        sock = stackweave.sync.tcp_connect(...)
        ...
    stackweave.sync.run(main)               # spawns main + drains
...
import stackweave_c
from stackweave.runtime import prewarm_stdlib as _prewarm_stdlib
...
def fiber(callable_, *args, **kwargs):
    ...
    if stackweave_c.current_g() is None:
        _prewarm_stdlib()
    if args or kwargs:
...
def run(main_fn=None):
    ...
    # Same prewarm as stackweave.runtime.run(): resolve getaddrinfo's lazy codec
    # import on the big stack before any fiber runs on a small one.
    _prewarm_stdlib()
    if main_fn is not None:
        stackweave_c.fiber(main_fn)
    return stackweave_c.run()
...
def current():
    ...
    return stackweave_c.current_g()


# --------------------------------------------------------------------
# Channels (re-export so callers don't need stackweave_c directly)
# --------------------------------------------------------------------
...
class Socket(object):
    ...
    def close(self):
        ...
        if fd >= 0:
            try: stackweave_c.netpoll_unregister(fd)
    ...
    def connect(self, addr):
        try:
            ...
            stackweave_c.wait_fd(self._s.fileno(), 2)
    ...
    def accept(self):
        while True:
            try:
                ...
                stackweave_c.wait_fd(self._s.fileno(), 1)
                continue
...
# --------------------------------------------------------------------
# Convenience constructors (mirror the async versions in stackweave.aio)
# --------------------------------------------------------------------
def tcp_connect(host, port, *, family=0, local_addr=None):
    """Connect a TCP socket cooperatively.  Returns a stackweave.sync.Socket."""
...
def udp_endpoint(local_addr=None, remote_addr=None, *, family=_socket.AF_INET,
    ...
    to remote_addr).  Returns a stackweave.sync.Socket."""
...
# --------------------------------------------------------------------
from stackweave.monkey import CoLock as Lock
from stackweave.monkey import CoEvent as Event
from stackweave.monkey import CoCondition as Condition
from stackweave.monkey._base import CoFMutex   # foreign-safe cooperative mutex (guard for the primitives below)
...
park = stackweave_c.park
park_self = stackweave_c.park_self
...
# zero fds per waiter.  A stackweave_c.Mutex (a cooperative fiber mutex) guards
...
# guard's wake path (stackweave_c.Mutex unlock -> mn_wake_g) is NOT foreign-OS-thread
...
def _acquire(mu):
    ...
    ``mu`` is a stackweave.monkey._base.CoFMutex.  acquire() handles the
    fiber-vs-foreign dispatch internally: a fiber waiter parks 0-fd
    (``stackweave_c.park(foreign_wakeable=True)``, which the single-thread drain
    ...
    this closes.  (Formerly ``mu`` was a channel-backed stackweave_c.Mutex; a foreign
...
class WaitGroup(object):
    ...
    def add(self, delta=1):
        ...
        # The guard is a stackweave_c.Mutex whose wake path is not foreign-OS-thread-
        ...
        if delta < 0 and stackweave_c.current_g() is None:
            raise RuntimeError(
    ...
    def wait(self):
        ...
        while True:
            ...
            self._mu.unlock()
            stackweave_c.set_wait_reason(stackweave_c.WR_WAITGROUP)
            stackweave_c.park()            # M:N-correct in-memory park (park_self
...
class Future(object):
    ...
    def _resolve(self, result, exc):
        # Resolving wakes parked awaiters; the guard is a stackweave_c.Mutex whose
    ...
    def result(self, timeout=None):
        ...
        self._waiters.append(g)
        self._mu.unlock()
        stackweave_c.set_wait_reason(stackweave_c.WR_FUTURE)
        while True:
            if deadline is None:
                stackweave_c.park()        # woken by _resolve() (wake-all + clear)
            else:
                ...
                if remaining <= 0:
                    ...
                    self._mu.unlock()
                    raise TimeoutError("Future.result timed out")
                stackweave_c.park(timeout=remaining)
            _acquire(self._mu)
            if self._done:
                exc, res = self._exc, self._result
                self._mu.unlock()
...
def gather(*callables):
    ...
    if not mn and stackweave_c.current_g() is None:
        raise RuntimeError(
            "gather() requires a running scheduler: call it from inside a fiber "
            "(stackweave.run()/mn_run()), or while an M:N run loop is active on "
    ...
    for i, fn in enumerate(callables):
        ...
        if mn:
            stackweave_c.mn_fiber(target)
        else:
            stackweave_c.fiber(target)
    wg.wait()
    for e in errs:
        if e is not None:
            raise e
...
# the stackweave_c.Mutex guard, following the same rules as WaitGroup / Future:
...
class Semaphore(object):
    ...
    def acquire(self, n=1, timeout=None):
        ...
        self._mu.unlock()
        stackweave_c.set_wait_reason(stackweave_c.WR_SEMAPHORE)
        while True:
            if deadline is None:
                stackweave_c.park()
            else:
